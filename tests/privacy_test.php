...
use \theme_roc\privacy\provider;

/**
 * Unit tests for theme_roc/classes/privacy/policy
 *
 * @copyright  2021 Peter Meint Heida
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_roc_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences() {
        ...
        $exportedpreferences = $writer->get_user_preferences('theme_roc');
        $this->assertCount(1, (array) $exportedpreferences);
        $this->assertEquals('false', $exportedpreferences->{provider::DRAWER_OPEN_NAV}->value);
        $this->assertEquals(get_string('privacy:drawernavclosed', 'theme_roc'),
        ...
        $this->assertEquals(get_string('privacy:drawernavopen', 'theme_roc'),
                $exportedpreferences->{provider::DRAWER_OPEN_NAV}->description);
    }
}
