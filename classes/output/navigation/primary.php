 ...
 * @package     core
 * @category    navigation
 * @copyright   2023 onwards Peter Meint Heida
...
class primary implements renderable, templatable {
    ...
    public function export_for_template(?renderer_base $output = null): array {
        ...
        if(is_admin()) {
            $menudata = (object) array_merge($this->get_primary_nav(),  $this->get_custom_menu($output));//Origin
        } else {
            $menudata = (object) $this->get_custom_menu($output);
        }
